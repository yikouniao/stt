...
void CLAUMp4TestDlg::OnBtnCapture() 
{
	...
	if(s_jpegHandle != -1)
		VSNET_ClientJpegCapSingle(s_jpegHandle,pGet_SelChl()->m_iChlSn,100);//第二个参数要注意，值当前通道的通道序号，不是当前窗口号
}
...
void CLAUMp4TestDlg::PTZActSafeCtrl(int type, int val)	//控制云台的运动
{
	...
	if(m_hChlLnk)
	{
		BOOL bSuc = VSNET_ClientPTZCtrl(m_hChlLnk, type, val);//真正控制运动的库函数

...
BOOL CLAUMp4TestApp::InitInstance()
{
	...
	char lang[10];
	char langinifile[256];
	sprintf(langinifile,"%s\\lang.ini",currdir);
	GetPrivateProfileString("LANGUAGE","showLang","chinese",lang,10,langinifile);
//	GetPrivateProfileString("LANGUAGE","Lang","chinese",lang,10,"\\lang.ini");
	if(strcmp(lang,"english") == 0)
	{
		m_hLonDll = LoadLibrary("en_resource.dll");
		if (m_hLonDll != NULL)
		{
			::AfxSetResourceHandle(m_hLonDll);
		}
	}
	else
	{
    ...
    /* NetDllHandle = LoadLibrary("C:/WINNT/system32/NetClient.dll");
	if(NetDllHandle == NULL)
	{
		TRACE("Load Library error %d\n",GetLastError());
	}*/
